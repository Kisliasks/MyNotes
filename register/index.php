...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if(isset($_POST['submit'])) {
    ...
    $input = validate_form_log_in();
    if(empty($input[0])) {

      $user = implode(', ',$input[1]);
     $_SESSION['user_name'] = $user;
     redirect("../index.php");

    } else {
      print implode(', ', $input[0]);
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(isset($_POST['register'])) {
      ...
      $input = user_register_validate();

      if(empty($input[0])) {
            
        user_register($input[1]);
        redirect("./");
...
<div class="container">
  <div class="frame">
    ...
    <div ng-app ng-init="checked = false">
          ...
          <input class="form-styling" type="text" name="user_password" placeholder=""/>
          <input type="checkbox" id="checkbox"/>
          <label for="checkbox" ><span class="ui"></span>Запомнить меня</label>
          <div class="btn-animate" >
            <!-- <a class="btn-signin">Войти</a>  -->
            <input type="submit" class="btn-signin" class="btn-animate" name="submit" value="Войти">
          </div>
				        </form>
          ...
          <div class="btn-animate" >
          <input type="submit" class="btn-signin" class="btn-animate" name="register" value="Зарегистрироваться">
          </div>
				        </form>
       ...
       viewBox="0 0 60 60" id="check" ng-class="checked ? 'checked' : ''">
                ...
                <div class="successtext">
                   <p> Вы зарегистрированы! Начните пользоваться напоминаниями!</p>
